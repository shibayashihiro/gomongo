...
@section('content')
    <div class="wd-md-rightbar table-rightblog">
        ...
        <div class="stock-body">
            <div class="">
                <table class="table stock-list-table table-striped w-100" id="servicesTable">
                    <thead>
                    <tr>
                        <th>ID</th>
                        <th>Title</th>
    ...
    <div class="modal fade wd-md-myexpense-popup" id="addService" tabindex="-1" role="dialog"
         aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content add_edit_service_modal_content">
...
@section('footer_script')
    <script>
        var dataTable = $('#servicesTable').DataTable({
            ...
            "order": [[0, "DESC"]],
            "bLengthChange": false,
            "ajax": "{{route('front.website.services.listing')}}",
            "language": {
                "paginate": {
                    "previous": "<",
                    "next": ">"
        ...
        function get_edit_form(id) {
            $.ajax({
                type: 'get',
                data: {'id': id},
                url: "{{route('front.website.services.get_edit_form')}}",
                beforeSend: addOverlay,
                success: function (r) {
                    removeOverlay();
                    $(".add_edit_service_modal_content").html(r);
            ...
            bootbox.confirm('{{__('Are you sure you want to delete?')}}', function (result) {
                if (result) {
                    $.ajax({
                        type: 'get',
                        data: {'id': id},
                        url: "{{route('front.website.services.delete')}}",
                        beforeSend: addOverlay,
                        success: function (r) {
                            removeOverlay();
                            if (r.status == 200) {
