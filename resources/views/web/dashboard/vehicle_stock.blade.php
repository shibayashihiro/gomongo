...
@section('content')
        ...
        <div class="stock-top-blog">
                ...
                <div class="wd-sl-gridftrs row">
                    ...
                    <div class="col-md-2">
                        <a href="javascript:;" class="wd-sl-ftrcount kt-iconbox--success kt-iconbox--animate-slower">
                            ...
                            <div class="wd-sl-textcounter">
                                <h3 id="total_additional_cost"></h3>
                                <h6>Total Additional Cost</h6>
                            </div>
                        </a>
                    </div>
                    <div class="col-md-2">
                        <a href="javascript:;" class="wd-sl-ftrcount kt-iconbox--secondry kt-iconbox--animate-slower">
                            <span>
                                <svg width="30" height="24" viewBox="0 0 30 24" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path
            ...
            <div class="wd-md-bottom wd-md-table"
                <div class="stock-body table-responsive">
                    <table class="table stock-list-table table-striped" id="vehicleStockTable">
                        <thead>
                        <tr>
                            <th>ID</th>
                            <th scope="col" class="r-number">Vehicle Registration Number</th>
                            <th scope="col" class="price">Purchase price</th>
                            <th scope="col" class="additional">Additional cost</th>
                            <th scope="col" class="additional">Total cost</th>
    ...
    <!-- pop-upfor view -->
    <div class="modal fade wd-md-myexpense-popup" id="viewStock" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body view_stock_modal_content">
    ...
    <div class="modal fade wd-md-myexpense-popup" id="addAdditionalStockPrice" tabindex="-1" role="dialog"
         aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog " role="document">
            <div class="modal-content add_additional_stock_price_modal_content">
...
@section('footer_script')
    <script>
        var dataTable = $('#vehicleStockTable').DataTable({
            "processing": true,
            "serverSide": true,
            "order": [[0, "DESC"]],
            "bLengthChange": false,
            "ajax": "{{route('front.vehicle_stock.listing')}}",
            "language": {
                "paginate": {
                    "previous": "<",
                    "next": ">"
            ...
            "columns": [
                ...
                {"data": "price"},
                {"data": "additional_price", searchable: false, sortable: false},
                {"data": "total_cost", searchable: false, sortable: false},
                {"data": "action", searchable: false, sortable: false}
            ],
            "drawCallback": function (settings) {
                $("#vehicleStockTable_info").append('<div class="wd-sl-custometotal"><div class="count-text wd-total">Total Cost <span>{{$total_stock}}</span></div></div>');
                $('#total_purchase_price').html(settings.json.filter_total.total_purchase_price);
                $('#total_additional_cost').html(settings.json.filter_total.total_additional_cost);
                $('#total_invested').html(settings.json.filter_total.total_invested);
            }
        });
        ...
        function get_edit_form(id) {
            $.ajax({
                type: 'get',
                data: {'id': id},
                url: "{{route('front.vehicle_stock.get_edit_form')}}",
            ...
            bootbox.confirm('{{__('Are you sure you want to delete?')}}', function (result) {
                if (result) {
                    $.ajax({
                        type: 'get',
                        data: {'id': id},
                        url: "{{route('front.vehicle_stock.delete_stock')}}",
                        beforeSend: addOverlay,
                        success: function (r) {
                            removeOverlay();
                            if (r.status == 200) {
        ...
        function get_view(id) {
            $.ajax({
                type: 'get',
                data: {'id': id},
                url: "{{route('front.vehicle_stock.get_view_form')}}",
                beforeSend: addOverlay,
                success: function (r) {
                    removeOverlay();
                    $(".view_stock_modal_content").html(r);
                    $("#viewStock").modal('show');
                }
            })
        }

        function get_additional_price_form(id) {
            $.ajax({
                type: 'get',
                data: {'id': id},
                url: "{{route('front.vehicle_stock.get_additional_price_form')}}",
                beforeSend: addOverlay,
                success: function (r) {
                    removeOverlay();
                    $(".add_additional_stock_price_modal_content").html(r);
                    $("#addAdditionalStockPrice").modal('show');
                }
            })
        }
    </script>
