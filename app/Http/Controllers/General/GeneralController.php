...
class GeneralController extends WebController
{
    ...
    public function Admin_dashboard(Request $request)
    {
        ...
        $transaction = UserSubscriptionPlan::where(['payment_status' => 'success'])->get();
        $graph_data = [];
        foreach ($transaction as $key => $value) {
            $graph_data[$key][] = (int)(strtotime($value->created_at) . '000');
            $graph_data[$key][] = ((double)$value->plan->price - (double)$value->plan->discount_price);
        }
    ...
    public function site_settings(Request $request)
    {
        ...
        foreach ($all_req as $key => $value) {
            ...
            if ($request->hasFile($key)) {
                $up = upload_file($key, 'admin_upload');
                if ($up) {
...
//        return redirect()->route('admin.login');
    ...
    {
        $user = User::find(base64_decode($token));
        if ($user) {
            $user->status = 'active';
            $user->save();
            success_session(__('User activate successfully'));
        }
        error_session('Invalid token');
