...
class NewUsersController extends WebController
{
    public function index()
    {
        return view('admin.new_user.index', [
            'title' => 'Users',
            'breadcrumb' => breadcrumb([
                'Users' => route('admin.new-user.index'),
            ]),
        ]);
    }
    ...
    public function listing()
    {
        ...
        $main = User::where('type', 'user')->doesntHave('DealerStock');
        ...
        if (!empty($all_data)) {
            foreach ($all_data as $key => $value) {
                $param = [
                    'id' => $value->id,
                    'url' => [
                        'status' => route('admin.new-user.status_update', $value->id),
                        'delete' => route('admin.new-user.destroy', $value->id),
                    ],
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.new-user.index');
    }
    ...
    public function show($id)
    {
        ...
        if ($data) {
            return view('admin.new_user.view', [
                'title' => 'View user',
                'data' => $data,
                'breadcrumb' => breadcrumb([
                    'user' => route('admin.new-user.index'),
                    'view' => route('admin.new-user.show', $id)
                ]),
            ]);
        }
        error_session('user not found');
    ...
    public function edit($id)
    {
        ...
        if ($data) {
            $title = "Update user";
            return view('admin.new_user.edit', [
                'title' => $title,
                'data' => $data,
                'breadcrumb' => breadcrumb([
                    'User' => route('admin.new-user.index'),
                    'edit' => route('admin.new-user.edit', $data->id)
                ]),
            ]);
        }
        error_session('user not found');
